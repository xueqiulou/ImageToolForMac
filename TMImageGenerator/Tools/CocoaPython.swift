...
@objc public class CocoaPython:NSObject {
    ...
    @objc public init(scrPath: String, args: [String]? = nil, complete: completeBlock? = nil) {
        completed = complete
...
//        let result = fetchResult(outPip)?.split(separator: "\n").map(String.init) ?? [""]
    ...
    @objc public func runAsync(asyncComlete: Bool = true) {
